...
public class BiomeCommandHandler {
    ...
    public static Component execute(Identifier biomeIdentifier, String script) {
        return GameUtils.getRegistryManager()
                .map(rm -> {
                    var biomeHolder = rm.lookup(Registries.BIOME).flatMap(r -> r.get(biomeIdentifier));
                    ...
                    var result = ContainerManager.resolve(IBiomeLibrary.class).eval(biomeHolder.get().value(), new Script(script));
