...
public class MorningFogRangeCalculator extends VanillaFogRangeCalculator {
    ...
    private boolean isFogAllowed() {
        return GameUtils.getWorld().map(w -> !w.dimensionType().hasFixedTime()).orElse(false);
    }

    private float getCelestialAngleDegrees() {
        return GameUtils.getWorld().map(w -> w.environmentAttributes().getDimensionValue(EnvironmentAttributes.SUN_ANGLE)).orElseThrow();
    }
