...
public final class ParticleUtils {
    ...
    public static SpriteSet getSpriteProvider(ParticleType<?> particleType) {
        var id = RegistryUtils.getRegistry(Registries.PARTICLE_TYPE)
                .flatMap(r -> r.getResourceKey(particleType).map(ResourceKey::identifier))
                .orElseThrow();
        ParticleResources resources = ((MixinParticleManager) GameUtils.getParticleManager()).dsurround_getParticleResources();
        return ((MixinParticleResources) (Object) resources).dsurround_getSpriteAwareFactories().get(id);
    }
