...
public class WarmToast  implements Toast {
    ...
    @Override
    public void update(@NotNull ToastManager toastComponent, long lastChanged) {
        if (this.changed) {
    ...
    @Override
    public void extractRenderState(@NotNull GuiGraphicsExtractor guiGraphics, @NotNull Font font, long lastChanged) {
        int i = this.width();
        ...
        if (this.messageLines.isEmpty()) {
            guiGraphics.text(font, this.title, 18, LINE_SPACING, this.profile.titleColor.getValue(), false);
        } else {
            guiGraphics.text(font, this.title, 18, 7, this.profile.titleColor.getValue(), false);

            for(int j = 0; j < this.messageLines.size(); ++j) {
                guiGraphics.text(font, this.messageLines.get(j), 18, 18 + j * LINE_SPACING, this.profile.bodyColor.getValue(), false);
            }
        }
    }
