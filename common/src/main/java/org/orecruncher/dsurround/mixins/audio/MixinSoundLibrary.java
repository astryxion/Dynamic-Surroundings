...
public class MixinSoundLibrary implements ISoundEngine {
    ...
    private long dsurround_buildCapabilities(long deviceHandle, IntBuffer attrList, Operation<Long> original) {
        if (AudioUtilities.doEnhancedSounds()) {
            // 26.1 already allocated a larger attribute buffer. Insert the EFX send count
     ...
     */
    @ModifyConstant(method = "init(Ljava/lang/String;Lcom/mojang/blaze3d/audio/DeviceList;Z)V", constant = @Constant(intValue = 8, ordinal = 0))
