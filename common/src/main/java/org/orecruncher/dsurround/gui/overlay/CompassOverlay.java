...
public class CompassOverlay extends AbstractOverlay {
    ...
    public void render(GuiGraphicsExtractor context, float partialTick) {
        ...
        try {
            ...
            this.drawTexture(context, COMPASS_TEXTURE, x, y, direction, v, BAND_WIDTH, BAND_HEIGHT);
    ...
    public void drawTexture(GuiGraphicsExtractor context, Identifier texture, float x, float y, float u, float v, float width, float height) {
        this.drawTexture(context, texture, x, x + width, y, y + height, width, height, u, v);
    }

    void drawTexture(GuiGraphicsExtractor context, Identifier texture, float x1, float x2, float y1, float y2, float regionWidth, float regionHeight, float u, float v) {
        this.drawTexturedQuad(context, texture, x1, x2, y1, y2, (float) 0, u, v, regionWidth, regionHeight);
    }

    void drawTexturedQuad(GuiGraphicsExtractor context, Identifier texture, float x1, float x2, float y1, float y2, float z, float u, float v, float regionWidth, float regionHeight) {
        int ix = Math.round(x1);
        int iy = Math.round(y1);
        int iw = Math.round(x2 - x1);
        int ih = Math.round(y2 - y1);
        context.blit(RenderPipelines.GUI_TEXTURED, texture, ix, iy, u, v, iw, ih, TEXTURE_SIZE, TEXTURE_SIZE);
    }
